...
def plotGeneStats(genetsspos, outpath, prefix):
    ...
    print(f"quantile of spearman cor: {np.quantile(generank['spearman_cor'], [0.25, 0.5, 0.75])}")
    plt.figure(figsize=(5, 4))
    plt.hist(generank['spearman_cor'], bins = 'auto', density=True)
    plt.xlabel('gene spearman correlation\nwith well positioned +1 nuc')
    plt.ylabel('density')
    plt.axvline(np.quantile(generank['spearman_cor'], 0.75), color = 'red')
    plt.savefig(outpath+f'{prefix}_gene_tss_spearman.pdf', bbox_inches='tight')
    plt.show()
    plt.close()
    
    
    print(f"quantile of pearson cor: {np.quantile(generank['pearson_cor'], [0.25, 0.5, 0.75])}")
    plt.figure(figsize=(5, 4))
    plt.hist(generank['pearson_cor'], bins = 'auto', density=True)
    plt.xlabel('gene pearson correlation\nwith well positioned +1 nuc')
    plt.ylabel('density')
    plt.axvline(np.quantile(generank['spearman_cor'], 0.75), color = 'red')
    plt.savefig(outpath+f'{prefix}_gene_tss_pearson.pdf', bbox_inches='tight')
    plt.show()
    plt.close()
    
    print(f"quantile of coverage: {np.quantile(generank['coverage'], [0.25, 0.5, 0.75])}")
    plt.figure(figsize=(5, 4))
    plt.hist(generank['coverage'], bins = 'auto', density=True)
    plt.xlabel('gene coverage')
    plt.ylabel('density')
    plt.axvline(np.quantile(generank['coverage'], 0.75), color = 'red')
    plt.savefig(outpath+f'{prefix}_gene_tss_coverage.pdf', bbox_inches='tight')
    plt.show()
    plt.close()
    
    print(f"quantile of variation: {np.quantile(generank['variation'], [0.25, 0.5, 0.75])}")
    plt.figure(figsize=(5, 4))
    plt.hist(generank['variation'], bins = 'auto', density=True)
    plt.xlabel('heterogeneity per gene')
    plt.ylabel('density')
    plt.axvline(np.quantile(generank['variation'], 0.75), color = 'red')

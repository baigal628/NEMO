                ...
                if readname in readchunks[chunkpos]:
                    ...
                    while mvpos < len_mv:
                        ...
                        if mv_val == 1 or mv_val == len_mv-1:
                            if kmer_idx in queryrefpospairs and queryrefpospairs[kmer_idx] != None and 3 <= kmer_idx < len(seq)-3:
                                # kmersigpos.append((start_signal_idx, start_signal_idx+currsiglen, queryrefpospairs[kmer_idx]))
                                if strand == -1:
                                    kmersigpos.append((start_signal_idx, start_signal_idx + currsiglen, len(seq) - kmer_idx, revcomp(seq[kmer_idx - 3:kmer_idx + 3]), queryrefpospairs[kmer_idx], refseq[queryrefpospairs[kmer_idx] - alignstart]))  # (kmer_idx, start_signal_idx, start_signal_idx+currsiglen))
                                else:
                                    kmersigpos.append((start_signal_idx, start_signal_idx + currsiglen, kmer_idx, seq[kmer_idx - 3:kmer_idx + 3], queryrefpospairs[kmer_idx], refseq[queryrefpospairs[kmer_idx] - alignstart]))  # (kmer_idx, start_signal_idx, start_signal_idx+currsiglen))
